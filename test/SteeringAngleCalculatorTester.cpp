...
TEST_CASE("Test calculation for steering angle") {
    ...
    testCones.blue.first.position = cv::Point(0,1);
    testCones.blue.second.position = cv::Point(0,0);
    testCones.yellow.first.position = cv::Point(640,1);
    testCones.yellow.second.position = cv::Point(640,0);
    ...
    SECTION("Test valid for invalid angle") {
        testCones.blue.first.position = cv::Point(0,0);
        testCones.blue.second.position = cv::Point(0,0);
        testCones.yellow.first.position = cv::Point(0,0);
        testCones.yellow.second.position = cv::Point(0,0);
        steeringAngle = SteeringAngleCalculator::calculateSteeringAngle(testCones);
...
TEST_CASE("Test find angles for cones") {
    std::pair<cone, cone> cones;
    const int EXPECTED_ANGLE = 90;
    double angle;
    cones.first.position = cv::Point(1,1);
    cones.second.position = cv::Point(1,0);
...
TEST_CASE("Test steering angle") {
    ...
    SECTION("Test steeringAngle with angle less than 90 degrees") {
        testCones.blue.first.position = cv::Point(0,2);
        testCones.blue.second.position = cv::Point(1,1);
        steeringAngle = SteeringAngleCalculator::outputSteeringAngle(LAST_STEERING_ANGLE,testCones,distanceReading);
        REQUIRE(steeringAngle < 0);
    }
    SECTION("Test steeringAngle with angle more than 90 degrees") {
        testCones.blue.first.position = cv::Point(1,2);
        testCones.blue.second.position = cv::Point(0,1);
        steeringAngle = SteeringAngleCalculator::outputSteeringAngle(LAST_STEERING_ANGLE,testCones,distanceReading);
        REQUIRE(steeringAngle > 0);
    }
}
